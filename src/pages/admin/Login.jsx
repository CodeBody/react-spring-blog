...
/**
 * 后台登录页面。
 * @returns {JSX.Element} 返回管理员登录界面。
 * @description 负责处理后台身份验证和登录后跳转。
 */
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden bg-[#050505]">
      <div className="absolute inset-0 z-0 select-none pointer-events-none">
        <MotionDiv initial={{ scale: 1.1, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 2.5, ease: [0.16, 1, 0.3, 1] }} className="w-full h-full relative">
          <img src="/assets/images/login-abstract.jpg" alt="Login Background" className="w-full h-full object-cover opacity-40" />
      ...
      <div className="max-w-7xl w-full px-8 lg:px-20 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center relative z-10">
        <MotionDiv initial={{ opacity: 0, x: -60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.5, delay: 0.3, ease: [0.16, 1, 0.3, 1] }} className="hidden lg:flex flex-col space-y-10">
          <div className="space-y-6">
            ...
            <p className="text-white/40 font-black tracking-[0.4em] text-xs uppercase pt-2">The Architecture of Digital Expression</p>
          </div>
        ...
        <MotionDiv initial={{ opacity: 0, scale: 0.95, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 1.2, delay: 0.5, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-[480px] mx-auto lg:ml-auto lg:mr-0">
          <div className="glass-card p-12 rounded-[3.5rem] shadow-2xl relative overflow-hidden group border-white/5 bg-white/[0.02] backdrop-blur-3xl">
            ...
            <div className="relative z-10">
              ...
              <form onSubmit={handleLogin} className="space-y-10">
                <div className="space-y-6">
                  <div className="relative group">
                    ...
                    <div className="relative">
                      <input type="text" placeholder="账号 ID" value={username} onChange={(event) => setUsername(event.target.value)} required className="w-full bg-white/[0.03] border-2 border-white/5 rounded-2xl py-4 px-6 text-white font-bold text-sm focus:outline-none focus:border-primary/50 transition-all placeholder:text-white/5 uppercase tracking-[0.1em] shadow-inner" />
                  ...
                  <div className="relative group">
                    ...
                    <div className="relative">
                      <input type="password" placeholder="••••••••" value={password} onChange={(event) => setPassword(event.target.value)} required className="w-full bg-white/[0.03] border-2 border-white/5 rounded-2xl py-4 px-6 text-white font-bold text-sm focus:outline-none focus:border-primary/50 transition-all placeholder:text-white/5 tracking-[0.5em] shadow-inner" />
                  ...
                  <AnimatePresence mode="wait">
                    {error && (
                      <MotionDiv initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="p-4 bg-destructive/10 border border-destructive/20 rounded-2xl flex items-center gap-3">
                ...
                <button type="submit" disabled={isLoading} className="w-full h-16 bg-primary text-primary-foreground rounded-2xl font-black tracking-[0.3em] text-xs uppercase flex items-center justify-center gap-4 transition-all duration-500 hover:shadow-[0_0_40px_-5px_rgba(99,102,241,0.5)] active:scale-95 disabled:opacity-50 relative group overflow-hidden">
          ...
          <MotionDiv initial={{ opacity: 0 }} animate={{ opacity: 0.2 }} transition={{ delay: 1.5 }} className="mt-10 text-center">
            <p className="text-[0.6rem] font-black uppercase tracking-[0.5em] text-white">© 2026 夏了个天 STUDIO · CORE v2.5.0</p>
          </MotionDiv>
        </MotionDiv>
      </div>
    </div>
